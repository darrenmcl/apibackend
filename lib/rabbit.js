...
async function connectRabbit(isRetry = false) {
    ...
    isConnecting = true;
    // Fix: Use AMQP_URL (the correct environment variable) with proper fallback
    ...
    logger.info(`[RabbitMQ] Using URL: ${url.replace(/\/\/[^:]+:[^@]+@/, '//***:***@')}`); // Log URL with masked credentials
...
// Export function to publish to chat queue (for compatibility)
